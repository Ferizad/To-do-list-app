...
const addItem = (task) => {
  list.innerHTML += `
    <li id="${task.index}" class="listTask">
      <div class="listTaskWrapper"><input type="checkbox" name="" class="checkbox" id=check${task.index} onclick="completedTodo(${todo.index})" ${todo.completed ? 'checked' : ''}></div>
      <input type="text" value="${task.description}" class="description edit">
      <button type="button" class="todoBtn" onclick="remove(${task.index})">
